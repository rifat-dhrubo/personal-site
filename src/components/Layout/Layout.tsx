...
export const Layout = ({ children }: Props) => {
  ...
  const [parent, enableAnimations] = useAutoAnimate<HTMLButtonElement>({
    duration: 300,
  });
  ...
  return (
    <div className="min-h-screen">
      <aside className="fixed top-0 left-1/2 mt-8 flex h-nav w-11/12 max-w-xl -translate-x-1/2  items-center justify-between rounded-xl border border-zinc-400/30 bg-zinc-50 p-4 backdrop-blur-sm dark:bg-zinc-800/60  md:left-0 md:mx-0 md:mt-0 md:h-full  md:w-nav md:max-w-none md:translate-x-0 md:flex-col md:rounded-none md:border-0 md:border-r md:py-8">
        <div
          id="top-nav"
